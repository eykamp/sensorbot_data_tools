---
title: "Create Map of Sensor Locations"
author: "Ian Eykamp"
date: "10/11/2021"
...
Creates a map of the Portland area with the locations of sensorbot
sensors, colored by the latest PM2.5 readings in the database.
...
pacman::p_load(pacman, RSQLite, DBI, ggmap, tidyverse, RColorBrewer, knitr)
...
Retrieves sensor locations and latest PM2.5 data from the `sensorbot.db`
database file.
...
sensorLocs <- dbGetQuery(conn, 
 "SELECT 
    name, attribute_key, attribute_kv.long_v, attribute_kv.dbl_v,
...
`get_map()` retrieves the map image to use as the background, from one
of three sources: Google Maps, OpenStreetMap, or Stamen Maps. Of these,
only Stamen Maps works out of the box; for Google Maps you must
configure an API key, which requires some a credit card number (though
it shouldn't cost much if anything to use).
